...
public class WatchtowerGenerator {
    private static final Identifier WATCHTOWER = ParadiseLost.locate("watchtower");
    ...
    public static void addPieces(StructureTemplateManager manager, StructurePiecesHolder structurePiecesHolder, BlockRotation blockRotation, BlockPos pos) {
        structurePiecesHolder.addPiece(new WatchtowerGenerator.Piece(manager, WATCHTOWER, pos, blockRotation));
    }

    public static class Piece extends SimpleStructurePiece {

        public Piece(StructureTemplateManager manager, Identifier template, BlockPos pos, BlockRotation rotation) {
            super(ParadiseLostStructureFeatures.WATCHTOWER_PIECE, 0, manager, template, template.toString(), createPlacementData(rotation), pos);
        }

        public Piece(StructureTemplateManager manager, NbtCompound nbt) {
            super(ParadiseLostStructureFeatures.WATCHTOWER_PIECE, nbt, manager, (identifier) -> createPlacementData(BlockRotation.valueOf(nbt.getString("Rot"))));
        }
